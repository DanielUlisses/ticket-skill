...
Delegate to the `ticket-tester` subagent. Call the Agent tool with `model: {{TEST_MODEL}}` explicitly. It discovers and runs the project's available checks without changing code. If a failure is caused by the change: `ticket-implementer` fixes it → `ticket-reviewer` reviews the delta → `ticket-tester` runs again (max 2 cycles). Pre-existing or environment failures are only reported.
