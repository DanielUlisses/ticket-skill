...
**They don't always stay together.** `gh pr merge --squash --delete-branch` takes the directory, git's worktree registration and the branch, and leaves the Herdr workspace — its tabs, its panes and its idle agent — with nothing on git's side left to find it by. That is the *ordinary* end of a ticket here, not an edge case, so the inventory is taken from **three** sources: git's worktrees, git's branches, and Herdr's own workspace list.
...
## Phase 2 — Report the four groups

Turn the rows into the four groups the developer actually thinks in, in this order, and say plainly when a group is empty:
...
Groups 1–3 are removed with `--worktree` or `--branch`; group 4 has a **different verb** and Phase 4 says which.
